...
std::vector<float> read_lidar_data(const std::string lidar_data_path)
{
    ...
    lidar_data_file.seekg(0, std::ios::end); //设置文件指针从文件尾部向前第0个字节，即文件尾部
    const size_t num_elements = lidar_data_file.tellg() / sizeof(float); //计算有多少个float数据
    lidar_data_file.seekg(0, std::ios::beg); //设置文件指针指向文件头部
